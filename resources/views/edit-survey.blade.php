...
<body>
       ...
       <div id="questionsContainer">
           @foreach ($questions as $index => $question)
               <div id="question_{{ $index }}" class="question-container">
                   ...
                   <div id="choicesContainer_{{ $index }}">
                       @foreach ($question->choices as $choiceIndex => $choice)
                           <div class="choice-container" id="choice_{{ $index }}_{{ $choiceIndex }}">
                               <label>Choice {{ $choiceIndex + 1 }}:</label>
                               <input type="text" name="questions[{{ $index }}][choices][{{ $choiceIndex }}][content]" value="{{ $choice->content }}" required>
                               <label>Next Question Content (optional):</label>
                               <select name="questions[{{ $index }}][choices][{{ $choiceIndex }}][next_question_id]">
                               ...
                               <button type="button" onclick="deleteChoice({{ $index }}, {{ $choiceIndex }})">Delete</button>
                           </div>
                       @endforeach
                   </div>
                   <button type="button" onclick="addChoice({{ $index }})">Add Choice</button>
   ...
   <script>
       let questionCount = {{ count($questions) }}; // Start with existing questions
       ...
       function addChoice(questionIndex) {
           const choicesContainer = document.getElementById(`choicesContainer_${questionIndex}`);
           const choiceCount = choicesContainer.childElementCount;
           const choiceContainer = document.createElement('div');
           choiceContainer.className = 'choice-container';
           choiceContainer.id = `choice_${questionIndex}_${choiceCount}`;
           choiceContainer.innerHTML = `
               <label>Choice ${choiceCount + 1}:</label>
               <input type="text" name="questions[${questionIndex}][choices][${choiceCount}][content]" required>
               <label>Next Question Content (optional):</label>
               <select name="questions[${questionIndex}][choices][${choiceCount}][next_question_id]">
                   <option value="">Select Next Question</option>
                   @foreach ($existingQuestions as $nextQuestion)
                       <option value="{{ $nextQuestion->id }}">
                           {{ $nextQuestion->content }}
               ...
               <button type="button" onclick="deleteChoice(${questionIndex}, ${choiceCount})">Delete</button>
           `;
       ...
       function deleteChoice(questionIndex, choiceIndex) {
           const choiceContainer = document.getElementById(`choice_${questionIndex}_${choiceIndex}`);
                   ...
                   <div id="choicesContainer_{{ $index }}">
                       @foreach ($question->choices as $choiceIndex => $choice)
                           <div class="choice-container" id="choice_{{ $index }}_{{ $choiceIndex }}">
                               ...
                               <!-- Hidden input to mark the choice for deletion -->
                               <input type="hidden" name="questions[{{ $index }}][choices][{{ $choiceIndex }}][delete]" value="0">
                               <button type="button" onclick="deleteChoice({{ $index }}, {{ $choiceIndex }})">Delete</button>
                           </div>
                       @endforeach
                   </div>
   ...
   <script>
       ...
       function addChoice(questionIndex) {
           ...
           choiceContainer.innerHTML = `
               ...
               <!-- Hidden input to mark the choice for deletion -->
               <input type="hidden" name="questions[${questionIndex}][choices][${choiceCount}][delete]" value="0">
               <button type="button" onclick="deleteChoice(${questionIndex}, ${choiceCount})">Delete</button>
           `;
       ...
       function deleteChoice(questionIndex, choiceIndex) {
           const choiceContainer = document.getElementById(`choice_${questionIndex}_${choiceIndex}`);
           // Mark the choice for deletion by changing the hidden input value
           const deleteInput = choiceContainer.querySelector('input[type="hidden"]');
           deleteInput.value = "1"; // Set the value to indicate deletion
           choiceContainer.style.display = 'none'; // Hide the choice container
       }
   </script>
</body>
</html> --}}
