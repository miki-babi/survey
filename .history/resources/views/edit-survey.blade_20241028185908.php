...
<body>
   <h1>Edit Survey</h1>
   <form id="surveyForm" action="{{ route('survey.update', $survey->id) }}" method="POST">
       @csrf
       @method('PUT') <!-- For updating the survey -->
       <div id="questionsContainer">
                   ...
                   <div id="choicesContainer_{{ $index }}">
                       @foreach ($question->choices as $choiceIndex => $choice)
                           <div class="choice-container">
                               ...
                               <label>Next Question ID (optional):</label>
                               <input type="number" name="questions[{{ $index }}][choices][{{ $choiceIndex }}][next_question_id]" value="{{ $choice->next_question_id }}">
                           </div>
                       @endforeach
                   </div>
   ...
   <script>
       let questionCount = {{ count($survey->questions) }}; // Start with existing questions
       ...
       function addChoice(questionIndex) {
           ...
           choiceContainer.innerHTML = `
               ...
               <label>Next Question ID (optional):</label>
               <input type="number" name="questions[${questionIndex}][choices][${choiceCount - 1}][next_question_id]">
           `;
